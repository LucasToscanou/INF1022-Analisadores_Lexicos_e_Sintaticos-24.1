...
import os
# Aqui se inicia a análise léxica, se todos os tokens pertecem à linguagem.

tokens = (
    'INICIO', 'MONITOR', 'EXECUTE', 'TERMINO', 'ENQUANTO', 'FACA', 'FIM', 
    'ID', 'EQUALS', 'NUMBER', 'ZERO', 'PLUS', 'MULTIPLY', 'LPAREN', 'RPAREN', 'COMMA',
    'IF', 'THEN', 'ELSE','EVAL', 'NEG', 'NEGATIVE', 'ENDIF'
)
...
# Aqui se encerra a análise léxica, se todos os tokens pertecem à linguagem.

#Aqui se inicia a análise sintática, se a combinação dos tokens nessa ordem pertencem à gramática da linguagem Provol-One.
...
def p_cmd(p):
    ...
    # POR QUE EH expr em vez de ID? no EVAL????????
    # ???????
    # ???
    ...
    if p[2] == '=':
        ...
        if len(p) == 8:
            ...
            p[0] = f"if ({p[2]}) {{\n{p[4]}\n}}"
    elif p[1] == 'EVAL' and len(p) == 9: # and (p[3] == 'ID' or p[3] == 'NUMBER') and (p[4] == 'ID' or p[4] == 'NUMBER'):
...
def traduzir_provol_one_para_c(arquivo_entrada, arquivo_saida):
...
for file in os.listdir("./inputs"):
    ...
    if file.endswith(".txt"):
        ...
        traduzir_provol_one_para_c(arquivo_entrada, arquivo_saida)
        print()

# arquivo_entrada = "./inputs/prog1.txt"       #Coloca aqui o caminho da entrada.
# arquivo_saida = "./outputs/prog1.c"           #Coloca aqui o caminho da saída.
