...
_lr_signature = 'leftPLUSMULTIPLYrightNEGCOMMA ELSE ENDIF ENQUANTO EQUALS EVAL EXECUTE FACA FIM ID IF INICIO LPAREN MONITOR MULTIPLY NEG NEGATIVE NUMBER PLUS RPAREN TERMINO THEN ZEROprograma : INICIO varlist MONITOR monitor EXECUTE cmds TERMINOmonitor : varlist\n               varlist : ID COMMA varlist\n               | IDcmds : cmd cmds\n            | cmdcmd : ID EQUALS expr\n           | ZERO LPAREN ID RPAREN\n           | ENQUANTO ID FACA cmds FIM\n           | IF expr THEN cmds ELSE cmds ENDIF\n           | IF expr THEN cmds ENDIF\n           | EVAL LPAREN expr COMMA expr COMMA cmds RPAREN\n           | EVAL cmds NUMBER FIMexpr : expr PLUS expr\n            | expr MULTIPLY expr\n            | LPAREN expr RPAREN\n            | NUMBER\n            | ID\n            | NEG expr %prec NEG'
...
_lr_productions = [
  ...
  ('programa -> INICIO varlist MONITOR monitor EXECUTE cmds TERMINO','programa',7,'p_programa','main.py',73),
  ('monitor -> varlist','monitor',1,'p_monitor','main.py',84),
  ('varlist -> ID COMMA varlist','varlist',3,'p_varlist','main.py',94),
  ('varlist -> ID','varlist',1,'p_varlist','main.py',95),
  ('cmds -> cmd cmds','cmds',2,'p_cmds','main.py',111),
  ('cmds -> cmd','cmds',1,'p_cmds','main.py',112),
  ('cmd -> ID EQUALS expr','cmd',3,'p_cmd','main.py',121),
  ('cmd -> ZERO LPAREN ID RPAREN','cmd',4,'p_cmd','main.py',122),
  ('cmd -> ENQUANTO ID FACA cmds FIM','cmd',5,'p_cmd','main.py',123),
  ('cmd -> IF expr THEN cmds ELSE cmds ENDIF','cmd',7,'p_cmd','main.py',124),
  ('cmd -> IF expr THEN cmds ENDIF','cmd',5,'p_cmd','main.py',125),
  ('cmd -> EVAL LPAREN expr COMMA expr COMMA cmds RPAREN','cmd',8,'p_cmd','main.py',126),
  ('cmd -> EVAL cmds NUMBER FIM','cmd',4,'p_cmd','main.py',127),
  ('expr -> expr PLUS expr','expr',3,'p_expr','main.py',171),
  ('expr -> expr MULTIPLY expr','expr',3,'p_expr','main.py',172),
  ('expr -> LPAREN expr RPAREN','expr',3,'p_expr','main.py',173),
  ('expr -> NUMBER','expr',1,'p_expr','main.py',174),
  ('expr -> ID','expr',1,'p_expr','main.py',175),
  ('expr -> NEG expr','expr',2,'p_expr','main.py',176),
]
